...
| SDK Hydra scoped-client secret (`sdk-hydra-scoped-secret`) | OAuth2 client secret | SDK integration tests — client scoped to a test-owned index, so tests need not borrow the shared fixture index | Dev | No | `projects/rust/sdk/tests/integration_tests.rs` (dev-only hardcoded fixture) |
