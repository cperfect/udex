...
* **Observability Fixture** (part of [`projects/compose/`](../projects/compose/README.md#observability)): an always-on, ClickHouse-backed OpenTelemetry pipeline (OTel Collector + ClickHouse + Vector + a HyperDX UI) that receives the telemetry the server, SDK, and CLI emit. See [Observability](#observability) below. Not part of a production deployment — Udex emits standard OTLP and can target any OTel-compatible backend.
...
* **Logs (hybrid)** — the server emits logs over OTLP when an endpoint is configured, and *always* also writes JSON to stdout. The stdout floor is a durable fallback that survives a Collector outage; Vector ships postgres/hydra container stdout into ClickHouse as a secondary floor.
...
flowchart LR
    ...
    app -. "JSON stdout" .-> vec["Vector"]
    pg[("PostgreSQL")] -. "container stdout" .-> vec
    col -->|"traces/metrics/logs"| ch[("ClickHouse")]
    vec -->|"container logs"| ch
    ch --> hdx["HyperDX (UI)"]
```
...
The fixture is **part of the base `projects/compose` stack** — it comes up with PostgreSQL and Hydra, always-on (in the devcontainer and in CI), with nothing extra to start. ClickHouse is the unified store; the collector ingests OTLP and also scrapes PostgreSQL metrics; HyperDX is a reader-only UI at <http://localhost:8080>. Starting the backends imposes no overhead on the application, which only emits OTLP when configured. The k3d dev deployment runs with observability **enabled** and full sampling, exporting to the compose collector via `host.k3d.internal` (the same host bridge used for PostgreSQL and Hydra). For component details and usage, see [projects/compose/README.md#observability](../projects/compose/README.md#observability).
...
| Observability tests | `obs` (non-k8s), `test_obs_k8s_` | Assert telemetry lands in ClickHouse. The always-run `obs` binary stands up a local OTLP-exporting server on every `cargo test`; the `test_obs_k8s_` tests assert the k3d deployment's telemetry lands (gated on `K8S_SERVER_URL`). ClickHouse is a hard dependency — these fail, never skip | Supplementary — telemetry coverage |
