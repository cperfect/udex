...
# Pod's own cert (re-encrypted backend hop) and Traefik's edge cert (client-facing
# TLS terminated at the ingress). Both are generated by gen-keys-and-certs.sh.
CERTS_DIR="${WORKSPACE_DIR}/projects/rust/server/tests/certs"
EDGE_CERTS_DIR="${WORKSPACE_DIR}/projects/k8s/traefik/certs"
# OTLP CA — trusted by the pods when exporting telemetry to the local Collector.
OTLP_CERTS_DIR="${WORKSPACE_DIR}/projects/observability/certs"
RELEASE_NAME="udex"
NAMESPACE="default"
...
for cert_file in \
  ...
  "${EDGE_CERTS_DIR}/tls.crt" "${EDGE_CERTS_DIR}/tls.key" \
  "${OTLP_CERTS_DIR}/ca.crt"; do
  if [[ ! -f "${cert_file}" ]]; then
...
helm upgrade --install "${RELEASE_NAME}" "${CHART_DIR}" \
  ...
  --set-file "secrets.traefikTlsCrt=${EDGE_CERTS_DIR}/tls.crt" \
  --set-file "secrets.traefikTlsKey=${EDGE_CERTS_DIR}/tls.key" \
  --set-file "secrets.otlpCa=${OTLP_CERTS_DIR}/ca.crt"
rm -f "${db_url_tmp}"
