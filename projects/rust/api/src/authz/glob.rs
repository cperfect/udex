...
fn compute_match(
    ...
    if current_pattern == "*" {
        ...
        return dp_match(pattern_segs, input_segs, p_idx + 1, i_idx + 1, memo);
        ...
        if i_idx < input_segs.len() {
            return dp_match(pattern_segs, input_segs, p_idx, i_idx + 1, memo);
        }
        
        return false;
    } else if current_pattern.contains("**") {
        // Enhanced handling for embedded ** patterns (like "quux**", "**suffix", "pre**suf")
        return handle_embedded_double_wildcard(pattern_segs, input_segs, p_idx, i_idx, memo);
    } else {
...
fn matches_segment<'a>(pattern: &'a str, input: &'a str) -> (bool, Option<&'a str>) {
    ...
    return matcher(parts, false);
}

/// Optimized version of matches_segment that works on a range of input segments without concatenation
fn matches_segment_range<'a>(pattern: &'a str, input_segs: &'a [&'a str], start_idx: usize, end_idx: usize) -> (bool, Option<&'a str>) {
    if pattern == "*" {
        return (true, None); // Matches anything
    } else if pattern == "**" {
        return (true, Some(pattern)); // Matches anything and could match more
    ...
    return matcher_range(parts, false);
}
...
impl<'a> VirtualConcatenatedInput<'a> {
    fn new(segments: &'a [&'a str], start_idx: usize, end_idx: usize) -> Self {
        ...
        let total_len = if end_idx > start_idx {
            segments[start_idx..end_idx].iter().map(|s| s.len()).sum::<usize>() + (end_idx - start_idx - 1)
        } else {
            0
        };
    ...
    fn starts_with(&self, prefix: &str) -> bool {
        ...
        for (i, &segment) in self.segments[self.start_idx..self.end_idx].iter().enumerate() {
    ...
    fn ends_with(&self, suffix: &str) -> bool {
        ...
        for (i, &segment) in self.segments[self.start_idx..self.end_idx].iter().rev().enumerate() {
    ...
    fn find(&self, needle: &str, start_pos: usize) -> Option<usize> {
        ...
        virtual_str[start_pos..].find(needle).map(|pos| start_pos + pos)
    }
    ...
    // Helper method for find and equals - only used when necessary
...
fn all_remaining_can_match_empty(pattern_segs: &[&str], start_idx: usize) -> bool {
    ...
    for i in start_idx..pattern_segs.len() {
        let segment = pattern_segs[i];
...
mod tests {
    ...
    fn test_matches_segment_range() {
        ...
        assert!(result == (false, None) || result == (true, None), 
    ...
    fn test_optimization_verification() {
        ...
        assert!(matches_glob("test:quux**", "test:quuxwibble:zip:baz"),
                "Embedded ** should span multiple segments");
        
        // Multiple embedded ** patterns
        assert!(matches_glob("test**:foo**:bar", "testABC:DEF:fooGHI:JKL:bar"),
                "Multiple embedded ** patterns should work");
        
        // Complex embedded ** with other wildcards
        assert!(matches_glob("pre**:*:suf**", "preABC:DEF:middle:sufGHI:JKL"),
