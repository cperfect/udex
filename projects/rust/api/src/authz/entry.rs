...
use crate::entry::{
    ...
    CreateEntryRequest, CreateEntryResponse,
    DeleteEntryRequest, DeleteEntryResponse,
    LookupContextByKeyRequest, LookupContextByKeyResponse,
    LookupKeysByContextRequest, LookupKeysByContextResponse,
    BulkWriteEntryOperationRequest, BulkWriteEntryOperationResponse,
    BulkReadEntryOperationRequest, BulkReadEntryOperationResponse,
};
use crate::authz::{
    permissions::{Permissable, is_permitted}, 
    claims::Claims
};
...
pub struct EntryServiceAuthorizor<E> where
    E: EntryService + Send + Sync + 'static,
{
   inner: Arc<E>,
}

impl<E> EntryServiceAuthorizor<E> where
    E: EntryService + Send + Sync + 'static,
{
...
impl Permissable<CreateEntryRequest> for CreateEntryRequest {
    fn required_permissions(&self) -> Vec<String> {
        vec![
            format!("udex:entry:v1:{}:create", self.index_name),
        ]
    }
}
...
impl Permissable<DeleteEntryRequest> for DeleteEntryRequest {
    fn required_permissions(&self) -> Vec<String> {
        vec![
            format!("udex:entry:v1:{}:delete", self.index_name),
        ]
    }
}
...
mod tests {
    ...
    async fn test_create_entry_with_valid_permissions() {
        ...
        mock_service
            .expect_create_entry()
            .times(1)
            .returning(|_| Ok(Response::new(CreateEntryResponse { key: "test-key".to_string(), context_hash: String::new() })));
    ...
    async fn test_delete_entry_with_valid_permissions() {
        ...
        let authorizor = EntryServiceAuthorizor::new(Arc::new(mock_service));
        let claims = create_test_claims_with_permissions(vec![format!("udex:entry:v1:{}:delete", "test-index").as_str()]);
    ...
    async fn test_lookup_context_by_key_with_valid_permissions() {
        ...
        mock_service
            ...
            .times(1)
            .returning(|_| Ok(Response::new(LookupContextByKeyResponse { index_name: String::new(), context: None })));

        let authorizor = EntryServiceAuthorizor::new(Arc::new(mock_service));
        let claims = create_test_claims_with_permissions(vec![format!("udex:entry:v1:{}:read", "test-index").as_str()]);
    ...
    async fn test_bulk_write_entry_operation_with_valid_permissions() {
        ...
        mock_service
            ...
            .times(1)
            .returning(|_| Ok(Response::new(BulkWriteEntryOperationResponse { results: vec![] })));

        let authorizor = EntryServiceAuthorizor::new(Arc::new(mock_service));
        let claims = create_test_claims_with_permissions(vec![format!("udex:entry:v1:{}:write", "test-index").as_str()]);
    ...
    async fn test_bulk_read_entry_operation_with_valid_permissions() {
        ...
        mock_service
            ...
            .times(1)
            .returning(|_| Ok(Response::new(BulkReadEntryOperationResponse { results: vec![] })));
    ...
    async fn test_wrong_permission_type() {
        let mock_service = MockEntryServiceImpl::new();
        let authorizor = EntryServiceAuthorizor::new(Arc::new(mock_service));
        let claims = create_test_claims_with_permissions(vec![format!("udex:index:v1:{}:read", "test-index").as_str()]); // wrong service
    ...
    async fn test_wrong_index_in_permission() {
        let mock_service = MockEntryServiceImpl::new();
        let authorizor = EntryServiceAuthorizor::new(Arc::new(mock_service));
        let claims = create_test_claims_with_permissions(vec![format!("udex:entry:v1:{}:read", "wrong-index").as_str()]);
