...
pub struct IndexServiceAuthorizor<I> where
    I: IndexService + Send + Sync + 'static,
{
   inner: Arc<I>,
}

impl<I> IndexServiceAuthorizor<I> where
    I: IndexService + Send + Sync + 'static,
{
...
impl Permissable<DescribeRequest> for DescribeRequest {
    fn required_permissions(&self) -> Vec<String> {
        vec![
            format!("udex:index:v1:{}:read", self.name)
        ]
    }
}
...
mod tests {
    ...
    async fn test_describe_with_valid_permissions() {
        ...
        let claims = create_test_claims_with_permissions(vec![format!("udex:index:v1:{}:read", "test-index").as_str()]);
    ...
    async fn test_create_index_without_permissions() {
        let mock_service = MockIndexServiceImpl::new();
        let authorizor = IndexServiceAuthorizor::new(Arc::new(mock_service));
        let claims = create_test_claims_with_permissions(vec![format!("udex:index:v1:{}:write", "test-index").as_str()]); // wrong permission
    ...
    async fn test_update_index_with_valid_permissions() {
        ...
        let authorizor = IndexServiceAuthorizor::new(Arc::new(mock_service));
        let claims = create_test_claims_with_permissions(vec![format!("udex:index:v1:{}:write", "test-index").as_str()]);
    ...
    async fn test_wrong_permission_type() {
        let mock_service = MockIndexServiceImpl::new();
        let authorizor = IndexServiceAuthorizor::new(Arc::new(mock_service));
        let claims = create_test_claims_with_permissions(vec![format!("udex:entry:v1:{}:read", "test-index").as_str()]); // wrong service
    ...
    async fn test_wrong_index_in_permissions() {
        ...
        let claims = create_test_claims_with_permissions(vec![format!("udex:index:v1:{}:read", "wrong-index").as_str()]);
