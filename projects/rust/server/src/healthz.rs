...
impl HealthzService {
    pub fn new(entry_service: Arc<dyn HealthCheck + Send + Sync>, index_service: Arc<dyn HealthCheck + Send + Sync>) -> Self {
        Self {
            entry_service,
            index_service,
        }
...
impl HealthzServiceTrait for HealthzService {
    ...
    async fn healthz(
        ...
        let entry_healthy = self.entry_service.is_healthy().await.map_err(|e| {
            Status::internal(format!("Entry service health check failed: {}", e))
        })?;

        // Check health of index service
        let index_healthy = self.index_service.is_healthy().await.map_err(|e| {
        ...
        Ok(Response::new(HealthzResponse {
            ...
            status_messages: vec![
                if entry_healthy { "Entry service is healthy".to_string() } else { "Entry service is unhealthy".to_string() },
                if index_healthy { "Index service is healthy".to_string() } else { "Index service is unhealthy".to_string() },
            ],
        }))
    }
}
...
mod tests {
    ...
    async fn test_healthz_both_services_healthy() {
        ...
        assert_eq!(healthz_response.status_messages[0], "Entry service is healthy");
        assert_eq!(healthz_response.status_messages[1], "Index service is healthy");
    ...
    async fn test_healthz_entry_service_unhealthy() {
        ...
        assert_eq!(healthz_response.status_messages.len(), 2);
        assert_eq!(healthz_response.status_messages[0], "Entry service is unhealthy");
        assert_eq!(healthz_response.status_messages[1], "Index service is healthy");
        assert!(healthz_response.server_time.is_some());
    }
    ...
    async fn test_healthz_index_service_unhealthy() {
        ...
        assert_eq!(healthz_response.status_messages.len(), 2);
        assert_eq!(healthz_response.status_messages[0], "Entry service is healthy");
        assert_eq!(healthz_response.status_messages[1], "Index service is unhealthy");
        assert!(healthz_response.server_time.is_some());
    }
    ...
    async fn test_healthz_entry_service_error() {
        ...
        mock_entry_service
            .expect_is_healthy()
            .times(1)
            .returning(|| Err(Error::ServerError("Entry service connection failed".to_string())));
        ...
        assert!(error.message().contains("Entry service health check failed"));
    ...
    async fn test_healthz_index_service_error() {
        ...
        mock_index_service
            .expect_is_healthy()
            .times(1)
            .returning(|| Err(Error::ServerError("Index service database unavailable".to_string())));
        ...
        let error = response.unwrap_err();
        assert_eq!(error.code(), tonic::Code::Internal);
        assert!(error.message().contains("Index service health check failed"));
        assert!(error.message().contains("Index service database unavailable"));
    }
