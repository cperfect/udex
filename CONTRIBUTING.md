...
Before building or running tests outside the devcontainer, generate the required
environment variables and key material:

```bash
bash scripts/gen-env.sh          # creates .env with DB passwords and Hydra secrets
bash scripts/gen-keys-and-certs.sh  # creates TLS certs and JWT signing keys under projects/rust/server/tests/
```

Both scripts are idempotent — re-running them when the outputs already exist is safe.
Pass `--force` to rotate existing values.

If you are using the devcontainer, both scripts run automatically during post-create.
