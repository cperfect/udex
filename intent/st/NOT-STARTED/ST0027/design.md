...
1. **Modular, not all-in-one.** The spike (`spike/clickstack-observability`, `projects/observability/spike-clickstack/FINDINGS.md`) showed the all-in-one is OpAMP/Mongo control-plane-driven, needs first-run setup, and gates OTLP behind a per-team bearer token — unfit for a deterministic git-tracked fixture. Owning the collector dissolves all three.
...
4. **TLS posture.** app -> collector stays TLS (collector terminates with the generated OTLP cert; app trusts the CA via `observability.otlp_ca`, unchanged from ST0026). collector -> ClickHouse and HyperDX -> ClickHouse are in-network plaintext for local dev — consistent with ST0026's internal Tempo/Loki/Prometheus hops.
5. **HyperDX always-on incl CI (per decision).** It adds weight and no test value in CI, so its readiness must **not gate** CI (start it, do not wait on it); any first-run setup must be pre-provisioned or irrelevant to test pass/fail.
6. **Log floor folded into the collector.** A `filelog`/docker receiver tails container stdout to preserve the ST0026 "durable floor" (postgres/hydra/app), retiring the Vector service. The docker-outside-of-docker log-path mount is the migration detail.
