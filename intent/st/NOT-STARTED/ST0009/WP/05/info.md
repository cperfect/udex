...
Update `udex-server`'s entry service gRPC handler to use the new 1:1 datastore API. The `lookup_keys_by_context` handler changes from returning a list to returning a single optional key. The `create_entry` handler relies on the datastore's idempotent upsert and returns the existing key without error on duplicate context. All callers of the old list-returning path are updated.
...
- [ ] `cargo build -p udex-server` passes with no warnings
