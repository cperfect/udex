...
manipulate Hydra's JWK key sets directly via the admin API. No additional mock
HTTP dependency is needed.

Test scenario:

1. Server starts configured against Hydra's JWKS endpoint; the initial key set
   is cached.
2. The test calls `jwk_api` to add a new key to Hydra's signing key set (or
   replaces the set entirely), triggering a `kid` that the server has not seen.
3. A JWT signed with the new key is presented; the server detects the unknown
   `kid`, refreshes from Hydra, and validates successfully.
4. Subsequent requests with the new key succeed without a second refresh.
