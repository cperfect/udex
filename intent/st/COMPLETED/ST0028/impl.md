...
The tolerance no longer costs loudness. `PendingReason` remembers *why* a poll kept coming back empty and, if the budget expires with the most recent attempt still rejected as not-yet-ingested, panics naming the stream or column — while a trailing success clears the memory, so a genuine "ingested but did not increase" still reports as the caller intends. It also records whether anything ever resolved, so the panic can distinguish "no attempt ever resolved" (a name is probably wrong) from "resolved earlier, then stopped" (weaker evidence). Cold start is tolerated during polling; a wrong name still fails loudly, just at the end instead of immediately. `IN-AG-NO-SILENT-001` holds.
