...
Located at `projects/rust/cli/Dockerfile`. Build context is `projects/rust/` (the Cargo workspace root) because the multi-crate workspace must be visible to the compiler.
...
2. **runtime** — `debian:bookworm-slim`. Copies only the binary. `EXPOSE 443`. `ENTRYPOINT ["udex", "serve", "--config", "/etc/udex/config.yaml"]`.
...
│           ├── deployment.yaml
│           ├── service.yaml
│           ├── configmap.yaml   (server config YAML)
...
The job is gated on paths: `projects/k8s/**`, `projects/rust/cli/Dockerfile`, `projects/rust/cli/**`, `projects/rust/sdk/**`. This avoids running a full cluster spin-up for unrelated changes.
...
**Build context is `projects/rust/`, not `projects/rust/cli/`.** Cargo requires the full workspace to resolve inter-crate dependencies. The Dockerfile path and context are independent; scripts pass `-f projects/rust/cli/Dockerfile projects/rust/`.
